...
var CSVHeaders = "Timestamp,OnChain,Type,Category,Amount(Msat),Amount(Fiat),TxID,Reference,BTCPrice,BTCTimestamp,FiatCurrency,Note"
...
func writeToCSV(e *frdrpc.ReportEntry) string {
	...
	return fmt.Sprintf("%v,%v,%v,%v,%v%v,%v%v,%v,%v,%v,%v,%v,%v",
		...
		amountPrefix, e.Fiat, e.Txid, e.Reference,
		e.BtcPrice.Price, e.BtcPrice.PriceTimestamp,
		e.BtcPrice.Currency, e.Note)
}
	...
	for i, line := range csvLines {
		if len(line) < 2 {
			return nil, errors.New("incorrect csv format. " +
				"At least two columns items are expected per " +
				"row")
		}
