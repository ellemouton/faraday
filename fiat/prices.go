...
type PriceSourceConfig struct {
	...
	// CSVPath is the path to the file where price data can be found. This
	// is used for the custom prices PriceBackend.
	CSVPath string

	// Currency is the currency code that the fiat prices should be quoted
	// in. This is currently used only for the custom prices PriceBackend.
	Currency string
}
...
func GetPrices(ctx context.Context, timestamps []time.Time,
	priceCfg *PriceSourceConfig) (map[time.Time]*Price, error) {
